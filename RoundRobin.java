...
public class RoundRobin{
	...
	public static void main(String[] args){
		...
		while(true){
			...
			for(int i = 0; (i < n) && (queue[i] != 0); i++){
				...
				if((temp_burst[queue[0]-1] == 0) && (complete[queue[0]-1] == false)){
					...
					completion[queue[0]-1] = timer; // Record completion time
		...
		System.out.print("\nProgram No.\tArrival Time\tBurst Time\tCompletion Time\tTurnAround Time\tWait Time"
						+ "\n");
		for(int i = 0; i < n; i++){
			System.out.print(i+1+"\t\t"+arrival[i]+"\t\t"+burst[i]
							+"\t\t"+completion[i]+"\t\t"+turn[i]+ "\t\t"+wait[i]+"\n");
		}
		for(int i =0; i< n; i++){
			avgWait += wait[i];
			avgTT += turn[i]; 
